...
object AuthActor {
  ...
  def apply(authService: AuthService)(implicit ec: ExecutionContext): Behavior[Command] = Behaviors.receive {
    (context, message) =>
      message match {
        ...
        case RefreshAccessToken(userId, refreshToken, replyTo) =>
          ...
          context.pipeToSelf(authService.refreshAccessToken(userId, refreshToken)) {
            case Success(result) => RefreshAccessTokenCompleted(result, replyTo)
            case Failure(ex)     => RefreshAccessTokenCompleted(Left(ex.getMessage), replyTo)
          }
          Behaviors.same

        case RefreshAccessTokenCompleted(Left(reason), replyTo) =>
          replyTo ! RefreshAccessTokenFailure(reason)
          Behaviors.same

        case RefreshAccessTokenCompleted(Right((accessToken, newRefreshToken)), replyTo) =>
